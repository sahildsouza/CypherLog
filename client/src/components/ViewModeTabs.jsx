      ...
      <div className="flex items-center gap-0.5 sm:gap-1.5 shrink-0">
        {tabs.map(tab => {
          ...
          return (
            ...
            >
              <Icon className={`w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0 ${isActive ? 'text-cyber-accent' : 'text-slate-400'}`} />
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 shrink-0 text-[10px] sm:text-xs">
        {metrics ? (
          <>
            {/* Speed Badge */}
            <div 
              className="flex items-center gap-1 px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-md bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-semibold shadow-glow-emerald shrink-0"
